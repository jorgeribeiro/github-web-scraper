...
def is_valid_repository(repository):
	...
	if len(repository) >= 3 and len(words) == 2:
		return True
	return False
...
def pull_file_content(url):
	...
	# Retorna soup da div que contém informações do arquivo
	response = request_url(url)
	soup = BeautifulSoup(response.text, 'html.parser')
	return soup.find(FILE_ELEMENT_FINDER, class_=FILE_CLASS_FINDER)

# print(pull_file_content('jorgimello/github-web-scraper/blob/master/repositories.txt'))
...
for r in repos:
	if not is_valid_repository(r):
		continue
	project_root_links = pull_folder_content(r)
